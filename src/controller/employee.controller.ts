...
class EmployeeController {
    ...
    addEmployee = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const addedEmployee = await this.employeeService.addEmployee(employeeDto as Employee)
    ...
    updateEmployeeById = async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if(errors.length > 0)
                throw new ValidationException(errors)
            const updatedEmployee = await this.employeeService.updateEmployeeById(employeeDto as Employee)
